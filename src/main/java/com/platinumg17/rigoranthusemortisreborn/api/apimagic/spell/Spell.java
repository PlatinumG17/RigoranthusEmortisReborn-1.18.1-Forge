...
public class Spell implements Cloneable{
    ...
    public static Spell deserialize(String recipeStr){
        ...
        for(String id : recipeList){
            if (RigoranthusEmortisRebornAPI.getInstance().getSpellpartMap().containsKey(id.trim()))
                recipe.add(RigoranthusEmortisRebornAPI.getInstance().getSpellpartMap().get(id.trim()));
        }
        return new Spell(recipe);
    }
