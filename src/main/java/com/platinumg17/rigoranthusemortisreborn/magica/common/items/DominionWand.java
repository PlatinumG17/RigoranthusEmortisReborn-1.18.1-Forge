...
public class DominionWand extends ModItem{
    ...
    public InteractionResult interactLivingEntity(ItemStack doNotUseStack, Player playerEntity, LivingEntity target, InteractionHand hand) {
        ...
        if((getPos(stack) == null || getPos(stack).equals(new BlockPos(0,0,0))) && getEntityID(stack) == -1){
            ...
            PortUtil.sendMessage(playerEntity, new TranslatableComponent("rigoranthusemortisreborn.dominion_wand.stored_entity"));
            return  InteractionResult.SUCCESS;
        }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if(getEntityID(stack) == - 1 && (getPos(stack) == null || getPos(stack).equals(new BlockPos(0,0,0)))){
            setPosTag(stack, pos);
            PortUtil.sendMessage(playerEntity, new TranslatableComponent("rigoranthusemortisreborn.dominion_wand.position_set"));
            return InteractionResult.SUCCESS;
        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag p_77624_4_) {
        ...
        tooltip.add(getEntityID(stack) == -1 ?
                new TranslatableComponent("rigoranthusemortisreborn.dominion_wand.no_entity") :
                new TranslatableComponent("rigoranthusemortisreborn.dominion_wand.entity_stored"));
        if(pos == null){
            tooltip.add(new TranslatableComponent("rigoranthusemortisreborn.dominion_wand.no_location"));
            return;
        }

        tooltip.add(new TranslatableComponent("rigoranthusemortisreborn.dominion_wand.position_stored", getPosString(pos)));
    }

    public static String getPosString(BlockPos pos){
        return new TranslatableComponent("rigoranthusemortisreborn.position", pos.getX(), pos.getY(), pos.getZ()).getString();
    }
}