...
public class EffectCut extends AbstractEffect {
    ...
    public void onResolveBlock(BlockHitResult rayTraceResult, Level world, @Nullable LivingEntity shooter, SpellStats spellStats, SpellContext spellContext) {
        for(BlockPos p : SpellUtil.calcAOEBlocks(shooter, rayTraceResult.getBlockPos(), rayTraceResult,spellStats.getBuffCount(AugmentAOE.INSTANCE), spellStats.getBuffCount(AugmentPierce.INSTANCE))) {
            ...
            Player entity = REFakePlayer.getPlayer((ServerLevel) world);
            ...
            if(world.getBlockEntity(p) != null && (world.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).isPresent() ||
                    world.getBlockEntity(p) instanceof SplinteredPedestalTile || world.getBlockEntity(p) instanceof TableTile))
                continue;
